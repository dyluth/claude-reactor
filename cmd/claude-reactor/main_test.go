...
func TestRunCommandFlags(t *testing.T) {
	tests := []struct {
		...
		{
			name:            "all flags set",
			args:            []string{"run", "--image", "full", "--account", "work", "--danger", "--shell", "--no-persist", "--no-mounts"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			var capturedDanger, capturedShell, capturedNoPersist, capturedNoMounts bool
			...
			testRunContainer := func(cmd *cobra.Command, app *pkg.AppContainer) error {
				...
				capturedNoMounts, _ = cmd.Flags().GetBool("no-mounts")
			...
			assert.Equal(t, tt.expectedNoMounts, capturedNoMounts)
		})
	}
}
...
// TestBuildCommandFlags tests build command flag parsing
func TestBuildCommandFlags(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name:            "default build",
			args:            []string{"build"},
			expectedVariant: "base", // Default variant
			expectedRebuild: false,
		},
		{
			name:            "build with variant",
			args:            []string{"build", "go"},
			expectedVariant: "go",
			expectedRebuild: false,
		},
		{
			name:            "build with rebuild flag",
			args:            []string{"build", "--rebuild"},
			expectedVariant: "base",
			expectedRebuild: true,
		},
		{
			name:            "build variant with rebuild",
			args:            []string{"build", "full", "--rebuild"},
			expectedVariant: "full",
			expectedRebuild: true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			app.Logger.(*mocks.MockLogger).On("Infof", mock.Anything, mock.Anything, mock.Anything).Return()
			
			// Mock the DockerManager.BuildImage method
			app.DockerMgr.(*mocks.MockDockerManager).On("BuildImage", mock.Anything, mock.AnythingOfType("string"), mock.AnythingOfType("string")).Return(nil)
			
			// Mock ArchDetector.GetDockerPlatform
			app.ArchDetector.(*mocks.MockArchDetector).On("GetDockerPlatform").Return("linux/arm64", nil)
			
			// Track captured values
			var capturedVariant string
			var capturedRebuild bool
			
			// Create build command with test implementation
			buildCmd := commands.NewBuildCmd(app)
			
			// Override the RunE to capture values without actually building
			buildCmd.RunE = func(cmd *cobra.Command, args []string) error {
				// Capture variant
				if len(args) > 0 {
					capturedVariant = args[0]
				} else {
					capturedVariant = "base"
				}
				
				// Capture rebuild flag
				capturedRebuild, _ = cmd.Flags().GetBool("rebuild")
				
				// Don't call the original function to avoid complex mocking
				app.Logger.Info("Building container images...")
				app.Logger.Infof("Building variant: %s, rebuild: %t", capturedVariant, capturedRebuild)
				return nil
			}
			
			// Create root command and add build command
			rootCmd := &cobra.Command{Use: "claude-reactor"}
			...
			assert.NoError(t, err)
			assert.Equal(t, tt.expectedVariant, capturedVariant)
			assert.Equal(t, tt.expectedRebuild, capturedRebuild)
		})
	}
}
